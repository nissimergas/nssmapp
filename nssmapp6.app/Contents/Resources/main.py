...
def get_information_accion_bolsa_santiago(driver, url,tiempo):
    driver.get(url)
    sleep(tiempo)
    dic={}
    soup=BeautifulSoup(driver.page_source, "html.parser")
    s=soup.find("div",attrs={"class": "col-lg-7 d-none d-sm-block"})

    sv=s.find("div",attrs={"class": "col-lg-3 border-right pr-2"})
    sact=s.find("div",attrs={"class": "col-lg-6 border-right"})
    valor=sv.find("h4",attrs={"class": "text-muted ng-binding"}).text
    s2=soup.find("div",attrs={"class": "card-header"})

    nombre= s2.find("h3",attrs={"class": "d-inline-block text-muted mb-0 font-weight-bold ng-binding"}).text
    nemo= s2.find("h3",attrs={"class": "text-secondary mb-0 ng-binding"}).text
    ultimaActualizacion=sact.find("h4").text
    dic["valor"]=valor
    dic["ultimaActualizacion"]=ultimaActualizacion
    dic["nombre"]=nombre
    dic["nemo"]=nemo
    return dic
...
class Gui():
    ...
    def push_acciones(self):
        ...
        #phantom="/Users/nissimergas/Desktop/dndpi/chromedriver"
        phantom="/Users/andresergas/Desktop/mcp/chromedriver"
        options = webdriver.ChromeOptions()
        options.add_argument('headless')
        #driver = webdriver.PhantomJS(executable_path=phantom)

        driver = webdriver.Chrome(executable_path=phantom, options=options)
        sleep(1)
    ...
    def push_bolsa_santiago(self):
        ...
        if direccion.strip()!=".xlsx":
            excel_document = openpyxl.load_workbook(filename=direccion)
            datos_links = excel_document['datos_bolsa_santiago']
            ...
            for n_fila in range(2,cantidad_datos+1):
                ...
                if ejecutar==1:
                    try:
                        url=datos_links.cell(row=n_fila, column=3).value
                        tiempo=int(datos_links['A4'].value)
                        dic=get_information_accion_bolsa_santiago(driver, url,tiempo)
                        nemo=dic["nemo"]
                        nombre=dic["nombre"]
                        valor=dic["valor"]
                        ultimaActualizacion=dic["ultimaActualizacion"]

                        tabla_precios.cell(row=n_fila, column=1).value = nemo
                        tabla_precios.cell(row=n_fila, column=2).value = nombre
                        tabla_precios.cell(row=n_fila, column=3).value = valor
                        tabla_precios.cell(row=n_fila, column=4).value = ultimaActualizacion
                    except:
                        tabla_precios.cell(row=n_fila, column=1).value ="error"
                        tabla_precios.cell(row=n_fila, column=2).value ="error"
                        tabla_precios.cell(row=n_fila, column=3).value ="error"
